...
const ProductList = async ({
  ...
  try {
    ...
    if (!categoryId) {
      throw new Error("Category ID is required but missing.");
    }
    ...
    return <p>An error occurred while loading products. Please try again later.</p>;
  }
};
...
const ProductList = async ({
  ...
  const productQuery = wixClient.products
    ...
    .hasSome(
      "productType",
      searchParams?.type ? [searchParams.type] : ["physical", "digital"] //change according to your needs
    )
    .gt("priceData.price", searchParams?.min || 0)
    .lt("priceData.price", searchParams?.max || 999999)
    .limit(limit || PRODUCT_PER_PAGE)
